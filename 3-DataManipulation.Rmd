...
```{r}
library(tidyr)
gayguides_growth <- gayguides_regions %>%
  select(state, Year, Division) %>%
  filter(Year == 1970 | Year == 1980) %>%
  group_by(Year, state, Division) %>%
  summarize(count = n()) %>%
  pivot_wider(names_from = Year, names_glue = "Year.{Year}", values_from = count, values_fill = 0) %>%
  mutate(growth = Year.1980 - Year.1970)
