...
> 3 < 4 tests whether 3 is less than 4. Since 3 is indeed less than 4, the return value is TRUE.
> 5 != 10 tests whether 5 is not equal to 10. Since 5 is indeed not equal to 10, the return value is TRUE.
> 5 != 5 tests whether 5 is not equal to 5. Since 5 is of course equal to itself, the return value is FALSE.
...
> Since I didn't know how to modify the code, I used ?head() to find the manual for this function. The examples showed that you could specify the number of rows using an additional argument, n = 6L, where 6 can be changed to whatever number you want. So I changed the 6 to 10 to retrieve the first 10 rows of data. The manual also showed examples for the tail() function, which logically made sense to find the last rows of a dataset. I used the same argument n = 6L to find the last six rows using tail().
...
> gayguides is the name of the dataset, which is a Data Frame. The dollar sign $ is the operator used to access the variables in the Data Frame. In this case we want to access the values in the title row, so we use $title. We use [500] to specify that we want the 500th value in the title row, which is "Rest Stop".
