...
rec.workers <- rec.workers %>%
  ...
  rename(c("male.paid" = "male paid", "female.paid" = "female paid", "men.volunteer" = "men volunteer", "women.volunteer" = "women volunteer")) %>%
  replace(is.na(.), 0) %>%
  rowwise() %>%
  mutate(paid.average = mean(c(male.paid, female.paid))) %>%
  mutate(volunteer.average = mean(c(men.volunteer, women.volunteer, volunteer))) %>%
  select(city, year, paid.average, volunteer.average) %>%
  mutate(year = str_replace(year, "X", "")) %>%
  mutate(year = as.integer(year))

ggplot(rec.workers, aes(x=year)) + 
  geom_line(aes(y = paid.average), color = "red") + 
  geom_line(aes(y = volunteer.average), color="blue") +
  labs(x="Year", y="Avg. Number of Volunteer & Paid Workers") +
  facet_wrap(~city)
```
