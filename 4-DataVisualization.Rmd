...
```{r}
BostonWomenVoters %>% subset(!is.na(Age)) %>% # Added to remove warning "Removed N rows containing non-finite values" which was preventing Knit
ggplot() +
  geom_histogram(mapping = aes(x=Age), color=4, fill="white")
```
...
BostonWomenVoters %>% subset(!is.na(Age)) %>% # Added to remove warning "Removed N rows containing non-finite values" which was preventing Knit
ggplot() +
  geom_histogram(mapping = aes(x=Age), color=4, fill="white", binwidth=10)
```

(@) Explain, what does changing the number of bins do? 

> I changed the binwidth instead of the number of bins. This increased the size of the age categories. Although the visualization is now less nuanced, it's easier to see the distribution of ages for the voters.
