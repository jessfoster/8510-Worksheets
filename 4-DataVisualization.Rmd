...
```
```{r}
ggplot(data = rec, mapping = aes(x = population, y=total_expenditures, color=state)) + geom_point()
```
...
gg.chart <- gayguides %>% 
  ...
  group_by(state) %>%
  summarize(count = n())
ggplot(data = gg.chart, mapping = aes(x = state, y = count)) +
 geom_col()
```
...
gg.religious <- gayguides %>%
  ...
  group_by(state) %>%
  summarize(count = n())
ggplot(data = gg.religious, mapping = aes(x = state, y = count)) +
  geom_col()
```
...
ggplot(data = nurses, mapping = aes(x = Country.of.Birth, y = count)) + # Tell ggplot to use nurses dataset and map variables Country.of.Birth to x-axis and count to y-axis.
...
gg.religious <- gayguides %>%
  ...
  group_by(Year) %>%
  summarize(count = n())
ggplot(data = gg.religious, mapping = aes(x = Year, y = count)) +
  geom_line()
```
...
ggplot(data = gg.risk, mapping = aes(x = Year, y = count)) +
  geom_line()
```
